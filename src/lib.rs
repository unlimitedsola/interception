...
use windows_sys::Win32::{
    ...
    System::{
        ...
        Registry::REGSTR_VAL_MAX_HCID_LEN,
...
impl Interception {
    ...
    pub fn new() -> Result<Self> {
        ...
        let mut wait_handles = Vec::with_capacity(MAX_DEVICES);

        for i in 0..MAX_DEVICES {
            let mut device = Device::new(i)?;
            // SAFETY: wait_handle and device are both stored in Interception,
            // so wait_handle is guaranteed to outlive device.
        ...
        // The loop above runs exactly MAX_DEVICES times.
        let devices = devices
            .try_into()
            .expect("Expected exactly MAX_DEVICES devices");
        let wait_handles = wait_handles
            .try_into()
            .expect("Expected exactly MAX_DEVICES wait handles");
    ...
    /// Returns a shared reference to all devices managed by this context.
    ...
    /// Set precedence for all devices in this context
    ///
    /// Higher precedence means earlier processing in the event chain, relative to other applications
    /// that also uses Interception to hook input events. Default precedence is 0.
    ///
    /// This method sets the same precedence value for all keyboard and mouse devices
    /// managed by this Interception context. This is useful when you want your entire
    /// application to have a consistent priority level relative to other input hooking
    /// applications.
...
pub const FILTER_KEY_E1: KeyFilter = 0x10;
...
pub struct KeyStroke {
    ...
    _unit_id: c_ushort,
    /// Scan code (`make_code` in Windows API)
    pub code: c_ushort,
    /// Key state flags
    pub state: KeyState,
...
impl Device {
    /// Creates a device by global index (`0..MAX_DEVICES`).
    ///
    /// Indices `0..MAX_KEYBOARD` are keyboards; `MAX_KEYBOARD..MAX_DEVICES` are mice.
    ...
    ///
    /// Higher precedence means earlier processing in the event chain.
    pub fn set_precedence(&mut self, precedence: Precedence) -> Result<()> {
        match self {
    ...
    /// Returns the list of hardware IDs reported by this device.
    pub fn get_hardware_ids(&mut self) -> Result<Vec<OsString>> {
        match self {
            Device::Keyboard(device) => device.get_hardware_ids(),
            Device::Mouse(device) => device.get_hardware_ids(),
        }
    }
}
...
impl KeyboardDevice {
    ...
    /// Get hardware IDs for this keyboard device
    pub fn get_hardware_ids(&mut self) -> Result<Vec<OsString>> {
        self.0.get_hardware_ids()
    }
}
...
impl MouseDevice {
    ...
    /// Get hardware IDs for this mouse device
    pub fn get_hardware_ids(&mut self) -> Result<Vec<OsString>> {
        self.0.get_hardware_ids()
    }
}
...
impl RawDevice {
    ...
    fn get_hardware_ids(&mut self) -> Result<Vec<OsString>> {
        // The returned hardware ID is a multi-string (REG_MULTI_SZ) value, and the character
        // limit is `REGSTR_VAL_MAX_HCID_LEN` as documented here:
        // <https://learn.microsoft.com/en-us/windows-hardware/drivers/install/hardware-ids#obtaining-the-list-of-hardware-ids-for-a-device>
        let mut buf = vec![0u16; REGSTR_VAL_MAX_HCID_LEN as usize];
        ...
        let len = (out_sz as usize) / size_of::<u16>();
        buf.truncate(len);

        /// Parse a [`REG_MULTI_SZ`] value into a vector of `OsString`
        ///
        /// [`REG_MULTI_SZ`]: https://learn.microsoft.com/en-us/windows/win32/sysinfo/registry-value-types
        fn parse_hardware_id(buf: &[u16]) -> Result<Vec<OsString>> {
            // Multi-string value must be terminated by an empty null-terminating string.
            if buf.is_empty() || buf.last().copied() != Some(0) {
                return Err(InterceptionError::MalformedHardwareId);
            }
            let mut ids = Vec::new();
            let mut start = 0;
            for (i, &c) in buf.iter().enumerate() {
                if c == 0 {
                    if start < i {
                        let os_str = OsString::from_wide(&buf[start..i]);
                        ids.push(os_str);
...
impl RawDeviceHandle {
    ...
    fn ioctl<In: ?Sized, Out: ?Sized>(
        ...
        let (input_ptr, input_size) = match input {
            Some(data) => (data as *const In as *const c_void, size_of_val(data) as u32),
            None => (ptr::null(), 0),
        };

        let (output_ptr, output_size) = match output {
            Some(data) => (data as *mut Out as *mut c_void, size_of_val(data) as u32),
            None => (ptr::null_mut(), 0),
        };
...
/// An owned Windows event handle used to signal when device input is available.
#[derive(Debug)]
#[repr(transparent)]
...
fn wait(handles: &[WaitHandle], timeout: Option<Duration>) -> Result<usize, WaitError> {
    if handles.is_empty() {
...
impl Display for InterceptionError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::MalformedHardwareId => write!(f, "Malformed hardware ID"),
